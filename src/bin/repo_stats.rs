...
println!("Cloning repository from {} into {}", repo_url, temp_dir.path().display());
...
fn count_files_and_lines(path: &Path) -> (usize, Vec<(String,usize)>) {
 ...
 for dir_entry in WalkDir::new(path).into_iter().filter_map(|e| e.ok()) {
    if dir_entry.path().extension().map_or(false, |ext| ext == "rs") {
        file_count += 1;
        let file_path = dir_entry.path().strip_prefix(path).unwrap().to_str().unwrap().to_string();
